...
# Must be PASSED (caller's responsibility) but may be empty — e.g. a full-
# residency run that adds no extra server flags.  ':?' would reject an empty
# string, so use '?' (errors only when the arg is entirely omitted).
ARGS="${2?need server args (may be '' for no extra flags), e.g. '--cpu-moe --prefill-metal-phases auto'}"
ENV_PREFIX="${3:-}"
...
WORKDIR="${WORKDIR:-$PWD}"
# ds4-server binary. Set it in q4-experiment.conf (it may live in a different
# repo than the models). Fallback: a sibling ds4 checkout next to scripts/.
DS4_SERVER_BIN="${DS4_SERVER_BIN:-$SCRIPTS_DIR/../ds4/ds4-server}"
...
# `${PREFIX}` is deliberately NOT baked into the default prompt: a per-run
# filename changes the prompt between runs (so they stop being comparable), and
# a >50-char filename makes the title sub-request's "<=50 chars + keep
# filenames exact" rules unsatisfiable -- the model then loops for 700-1000
# THINKING tokens recounting characters.  Per-run artifact uniqueness is
# restored by renaming the fixed-name output to <prefix>-minesweeper.html
# after the run.
...
PROMPT="${PROMPT:-create minesweeper. 1 html file. file name is minesweeper.html}"
...
# ---- 1. purge the macOS unified buffer cache (needs passwordless sudo) -------
...
if [ "$CLIENT" = "verify" ]; then
    ...
    # Output lands next to the run's logs (WORKDIR) so the verify text sits
    # beside <prefix>-ds4-server.log; pass VERIFY_REF=<that path> to diff.
    # The request json stays scratch in /tmp.
    VERIFY_OUT="${WORKDIR}/${PREFIX}-verify.txt"
...
PY
    ...
    oc_exit=$?
    # Per-run artifact uniqueness without baking the label into the prompt:
    # rename the fixed-name output to the prefixed name.
    for _d in "$WORKDIR" "$PWD"; do
        if [ -f "$_d/minesweeper.html" ]; then
            mv -f "$_d/minesweeper.html" "$_d/${PREFIX}-minesweeper.html"
            echo "[run] [$PREFIX] artifact -> $_d/${PREFIX}-minesweeper.html"
            break
        fi
    done
fi
