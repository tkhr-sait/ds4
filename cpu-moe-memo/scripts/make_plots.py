...
def build_mactop_net_disk(rows, title, phase_bands=None, annotations=None, x_max_override=None, session_footer=None, x_axis_format=None):
    ...
    net_in_mbs = [(r["t"], r["net_in_bps"] / MIB) for r in rows]
    net_out_mbs = [(r["t"], r["net_out_bps"] / MIB) for r in rows]
    return render_svg(
        title=title,
        x_label="elapsed seconds (mactop epoch)",
        series_left=[
        ...
        y_left_label="disk MB/s",
        series_right=[
            ("net in MB/s", COLORS[2], net_in_mbs),
            ("net out MB/s", COLORS[3], net_out_mbs),
        ],
        y_right_label="net MB/s",
        phase_bands=phase_bands, annotations=annotations, x_max_override=x_max_override, session_footer=session_footer, x_axis_format=x_axis_format,
    )
...
def process_session(
    ...
    if scenarios is None or len(scenarios) == 0:
        scenarios = discover_prefixes(session_dir)
        if not scenarios:
    ...
    for scenario in scenarios:
        ...
        if ds4_log.exists():
            ...
            write_svg(
                ...
                build_tok_plot(
                    ...
                    x_max_override=tok_x_max if tok_x_max > 0 else None,
                    session_footer=session_footer, x_axis_format="time",
                ),
            )
        io_xmax = iostat_x_max if iostat_x_max > 0 else None
        vm_xmax = vmstat_x_max if vmstat_x_max > 0 else None
        if io_log.exists():
            rows = parse_iostat(io_log, interval=iostat_interval)
            write_svg(
                plot_dir,
        ...
        if mactop_log.exists():
            ...
            if not rows:
                ...
                mac_xmax = mactop_x_max if mactop_x_max > 0 else None
                ...
                write_svg(plot_dir, f"{scenario}-mactop-net-disk.svg",
                          build_mactop_net_disk(rows, f"{scenario}: disk / network I/O", mactop_bands,
                                                annotations=mactop_annotations, x_max_override=mac_xmax,
                                                session_footer=session_footer, x_axis_format="time"))
...
def main():
    ...
    parser.add_argument(
        ...
        metavar="NAME",
        help=(
            "TEST_PREFIX to process (matches <NAME>-ds4-server.log etc.).  May "
            "be repeated.  Without --prefix, all prefixes present in "
            "session_dir are auto-detected."
        ),
    )
    parser.add_argument(
        "--iostat-interval",
